...
class Tombolone(tk.Frame):
    def __init__(self, master=None):
        ...
        self.screen="window"
        self.master.bind("<Control-f>", self.fullscreen)
    ...
    def colors(self):
        ...
        self.gioco_schedine_unset_dark_fg_color = "black"
        ...
        self.gioco_schedine_unset_ligth_bg_color = "white"
        self.gioco_schedine_unset_ligth_fg_color = "black"
        self.gioco_schedine_set_ligth_bg_color = "gray"
        self.gioco_schedine_set_ligth_fg_color = "white"
    ...
    def CreateWidgets(self):
        self.CreaClassifica()
        self.CreaMenuPrincipale()
        self.CreaLogIn()
        self.CreaSignup()
        self.CreaGiocoCPU()
        self.CreaGiocoGiocatori()
        self.CreaTipoGioco()
        self.CreaPreGiocoCPU()
        self.CreaPreGiocoGiocatori()
        self.CreaScegliNick()
        self.CreaImpostazioni()
    ...
    def CreaClassifica(self):
        ...
        self.lblTitoloClassifica=tk.Label(self.framePunteggio, text="Informazioni Profilo", bg=self.punteggio_titolo_bg_color, fg=self.punteggio_titolo_fg_color, font=('Helvetica', int((self.altezza/30) + (self.larghezza/54))))
        self.lblTitoloClassifica.grid(row=1, column=1)


        frameInfos=tk.Frame(self.framePunteggio, bg=self.punteggio_sfondo_bg_color)

        self.lblInfoNomeLogin=tk.Label(frameInfos, text="Username: ", font=("Helvetica", int((self.altezza / 51) + (self.larghezza / 91))), bg=self.punteggio_labels_bg_color, fg=self.punteggio_labels_fg_color)
        self.lblInfoNomeLogin.grid(row=0, column=0)

        self.enInfoUsrNameVar=tk.StringVar(self, self.giocatore.getNomeLogin())
        self.enInfoUsrName=tk.Entry(frameInfos, textvariable=self.enInfoUsrNameVar, font=("Helvetica", int((self.altezza / 51) + (self.larghezza / 91))), bg=self.punteggio_labels_bg_color, fg=self.punteggio_labels_fg_color)
        self.enInfoUsrName.grid(row=0, column=2)


        self.lblInfoNome = tk.Label(frameInfos, text="Nickname: ", font=("Helvetica", int((self.altezza / 51) + (self.larghezza / 91))), bg=self.punteggio_labels_bg_color,fg=self.punteggio_labels_fg_color)
        self.lblInfoNome.grid(row=1, column=0)

        self.enInfoNickNameVar = tk.StringVar(self, self.giocatore.getNome())
        self.enInfoNickName = tk.Entry(frameInfos, textvariable=self.enInfoNickNameVar, font=("Helvetica", int((self.altezza / 51) + (self.larghezza / 91))), bg=self.punteggio_labels_bg_color, fg=self.punteggio_labels_fg_color)
        self.enInfoNickName.grid(row=1, column=2)


        self.lblInfoPassword = tk.Label(frameInfos, text="Password: ", font=("Helvetica", int((self.altezza / 51) + (self.larghezza / 91))), bg=self.punteggio_labels_bg_color, fg=self.punteggio_labels_fg_color)
        self.lblInfoPassword.grid(row=2, column=0)

        self.btnInfoMostraPswd = tk.Button(frameInfos, text="Mostra", font=("Helvetica", int((self.altezza / 51) + (self.larghezza / 91))), highlightthickness=0, command=lambda:self.mostraPswdProfilo(), bg=self.punteggio_labels_bg_color, fg=self.punteggio_labels_fg_color)
        self.btnInfoMostraPswd.grid(row=2, column=2)

        self.pswdInfos = tk.StringVar(self, self.giocatore.getPassword())
        self.enPasswordInfos = tk.Entry(frameInfos, textvariable=self.pswdInfos, font=("Helvetica", int((self.altezza / 51) + (self.larghezza / 91))), bg=self.punteggio_labels_bg_color, fg=self.punteggio_labels_fg_color)


        self.lblInfoCrediti = tk.Label(frameInfos, text="Crediti: ", font=("Helvetica", int((self.altezza / 51) + (self.larghezza / 91))), bg=self.punteggio_labels_bg_color, fg=self.punteggio_labels_fg_color)
        self.lblInfoCrediti.grid(row=3, column=0)

        self.lblInfoCrediti2 = tk.Label(frameInfos, text=str(self.giocatore.getCrediti()), font=("Helvetica", int((self.altezza / 51) + (self.larghezza / 91))), bg=self.punteggio_labels_bg_color, fg=self.punteggio_labels_fg_color)
        self.lblInfoCrediti2.grid(row=3, column=2)

        frameInfos.grid(row=2, column=1)


        self.frameInsertPswdInfos = tk.Frame(self.framePunteggio, bg=self.impostazioni_sfondo_bg_color)

        self.lblTitoloInsPswd = tk.Label(self.frameInsertPswdInfos, text="Verifica Password", font=('Helvetica', int((self.altezza / 30) + (self.larghezza / 54))), bg=self.impostazioni_titolo_bg_color, fg=self.impostazioni_titolo_fg_color)
        self.lblTitoloInsPswd.grid(row=0, column=1, columnspan=2, pady=15)

        self.btnTornaAlMenuInsPswd = tk.Button(self.frameInsertPswdInfos, text="←", bg=self.impostazioni_titolo_bg_color, fg=self.impostazioni_titolo_fg_color, highlightthickness=0, command=lambda: self.Punteggio(event=None), relief="solid")
        self.btnTornaAlMenuInsPswd.grid(row=0, column=0, columnspan=2, sticky="w")

        self.lblPasswordInsPswd = tk.Label(self.frameInsertPswdInfos, text="Password", font=("Helvetica", int((self.altezza / 51) + (self.larghezza / 91))), bg=self.impostazioni_labels_bg_color, fg=self.impostazioni_labels_fg_color)
        self.lblPasswordInsPswd.grid(row=1, column=1, columnspan=2)

        self.passwordInsLogin = tk.StringVar(self, "")
        self.enPasswordInsPswd = tk.Entry(self.frameInsertPswdInfos, textvariable=self.passwordInsLogin, font=("Helvetica", int((self.altezza / 51) + (self.larghezza / 91))), width=20, bg=self.impostazioni_labels_bg_color, fg=self.impostazioni_labels_fg_color, highlightthickness=0)
        self.enPasswordInsPswd.grid(row=2, column=1, columnspan=2)

        self.btnVerificaInsPswd = tk.Button(self.frameInsertPswdInfos, text="Verifica", command=lambda:self.verificaPasswordInsPswd(), width=7, font=("Helvetica", int((self.altezza / 51) + (self.larghezza / 91))), bg=self.impostazioni_options_bg_color, fg=self.impostazioni_options_fg_color,highlightthickness=0)
        self.btnVerificaInsPswd.grid(row=3, column=1, columnspan=2)

        self.frameInsertPswdInfos.rowconfigure(1, weight=1)
        self.frameInsertPswdInfos.rowconfigure(2, weight=1)
        self.frameInsertPswdInfos.rowconfigure(3, weight=1)
        self.frameInsertPswdInfos.rowconfigure(4, weight=1)

        self.frameInsertPswdInfos.grid(row=2, column=1)


        self.framePunteggio.rowconfigure(2, weight=1)
        self.framePunteggio.columnconfigure(1, weight=1)

        self.framePunteggio.grid(row=0, column=0)


    def CreaMenuPrincipale(self):
        #TODO*****************************************************************************************************************\
        # *****************************************  MENU PRINCIPALE  ********************************************************\
        # *********************************************************************************************************************
        ...
        self.btnClassificaPersonale=tk.Button(self.frameMenu, text="Profilo", command=lambda:self.PunteggioSuono(), bg=self.menu_sfondo_bg_color, fg=self.menu_titolo_fg_color, font=('Helvetica', int((self.altezza/102) + (self.larghezza/182))), highlightthickness=0)
        self.btnClassificaPersonale.grid(row=0, column=0, columnspan=2, padx=15, pady=15, sticky="sw")
        ...
        self.btnLoginMenu=tk.Button(self.frameMenu, text="Login", command=lambda: self.Login(), bg=self.menu_buttons_bg_color, fg=self.menu_buttons_fg_color, font=('Helvetica', int((self.altezza/102) + (self.larghezza/182))), highlightthickness=0)
        self.btnLoginMenu.grid(row=0, rowspan=2, column=1, columnspan=2, padx=15, pady=15, sticky="ne")
    ...
    def CreaGiocoCPU(self):
        ...
        self.btnTornaAlMenuGiocoCPU=tk.Button(self.frameGiocoCPU, text="←", bg=self.gioco_sfondo_bg_color, fg=self.gioco_titolo_fg_color, font=("Helvetica", int((self.altezza/51) + (self.larghezza/91))), command=lambda:self.MenuSuono(), relief="solid", highlightthickness=0, bd=0)
        self.btnTornaAlMenuGiocoCPU.grid(row=0, column=0, columnspan=2, sticky="w")


        frameLogs=tk.LabelFrame(self.frameGiocoCPU, text=" Logs:", bg=self.gioco_logs_bg_color, fg=self.gioco_titolo_fg_color, highlightbackground=self.gioco_titolo_fg_color, highlightthickness=1, borderwidth=0)

        self.txtLogsCPU=tk.Text(frameLogs, width=45, height=6, highlightthickness=0, font=("Helvetica", int((self.altezza/85) + (self.larghezza/151))), state=tk.DISABLED, bd=0, bg=self.gioco_logs_bg_color, fg=self.gioco_logs_fg_color)
        ...
        frameDestro=tk.Frame(self.frameGiocoCPU, bg=self.gioco_framedestro_bg_color, highlightbackground=self.gioco_titolo_fg_color, highlightthickness=1)
        ...
        framePremiRimanenti=tk.LabelFrame(frameDestro, text=" Premi Disponibili:", bg=self.gioco_premi_bg_color, fg=self.gioco_premi_titolo_fg_color, highlightbackground=self.gioco_titolo_fg_color, highlightthickness=1, borderwidth=0)

        blank=tk.Label(framePremiRimanenti, text="", width=2, bg=self.gioco_premi_bg_color).grid(row=0, column=0)

        self.btnAmboCPU=tk.Button(framePremiRimanenti, text="AMBO", highlightthickness=0, height=2, width=8, font=("Helvetica", int((self.altezza/160) + (self.larghezza/280))), bg=self.gioco_premi_btns_bg_color, fg=self.gioco_premi_btns_fg_color, disabledforeground=self.gioco_premi_btns_fg_color, state=tk.DISABLED)
        self.btnAmboCPU.grid(row=1, column=1, pady=30, sticky="nesw")

        self.btnTernaCPU=tk.Button(framePremiRimanenti, text="TERNA", highlightthickness=0, height=2, width=8, font=("Helvetica", int((self.altezza/160) + (self.larghezza/280))), bg=self.gioco_premi_btns_bg_color, fg=self.gioco_premi_btns_fg_color, disabledforeground=self.gioco_premi_btns_fg_color, state=tk.DISABLED)
        ...
        self.btnQuaternaCPU=tk.Button(framePremiRimanenti, text="QUATERNA", highlightthickness=0, height=2, width=8, font=("Helvetica", int((self.altezza/160) + (self.larghezza/280))), bg=self.gioco_premi_btns_bg_color, fg=self.gioco_premi_btns_fg_color, disabledforeground=self.gioco_premi_btns_fg_color, state=tk.DISABLED)
        self.btnQuaternaCPU.grid(row=2, column=1, pady=30, sticky="nesw")

        self.btnCinquinaCPU=tk.Button(framePremiRimanenti, text="CINQUINA", highlightthickness=0, height=2, width=8, font=("Helvetica", int((self.altezza/160) + (self.larghezza/290))), bg=self.gioco_premi_btns_bg_color, fg=self.gioco_premi_btns_fg_color, disabledforeground=self.gioco_premi_btns_fg_color, state=tk.DISABLED)
        ...
        self.frameClassProvCPU=tk.LabelFrame(frameDestro, text=" Classifica:", bg=self.gioco_classifica_bg_color, fg=self.gioco_classifica_titolo_fg_color, highlightbackground=self.gioco_titolo_fg_color, highlightthickness=1, borderwidth=0)
        self.frameClassProvCPUIn=tk.Frame(self.frameClassProvCPU, bg=self.gioco_classifica_bg_color, highlightbackground=self.gioco_titolo_fg_color, highlightthickness=1, borderwidth=0)

        self.frameClassProvCPUIn.columnconfigure(0, weight=1)
        self.frameClassProvCPUIn.columnconfigure(1, weight=1)
        self.frameClassProvCPUIn.columnconfigure(2, weight=1)

        lblPos=tk.Label(self.frameClassProvCPUIn, text="Posizione", font=("Helvetica", int((self.altezza/128) + (self.larghezza/228)), "bold"), borderwidth=0.5, highlightbackground="black", relief=tk.SOLID, bg=self.gioco_classifica_bg_color, fg=self.gioco_classifica_tabella_fg_color).grid(row=0, column=0, sticky="nesw")
        lblNome=tk.Label(self.frameClassProvCPUIn, text="Nome", font=("Helvetica", int((self.altezza/128) + (self.larghezza/228)), "bold"), borderwidth=0.5, highlightbackground="black", relief=tk.SOLID, bg=self.gioco_classifica_bg_color, fg=self.gioco_classifica_tabella_fg_color).grid(row=0, column=1, sticky="nesw")
        lblPunt=tk.Label(self.frameClassProvCPUIn, text="Punteggio", font=("Helvetica", int((self.altezza/128) + (self.larghezza/228)), "bold"), borderwidth=0.5, highlightbackground="black", relief=tk.SOLID, bg=self.gioco_classifica_bg_color, fg=self.gioco_classifica_tabella_fg_color).grid(row=0, column=2, sticky="nesw")

        self.frameClassProvCPUIn.grid(row=0, column=0, padx=10, pady=10)
        ...
        # Tabellone
        #todo DA COMPATTARE
        frameTabellone=tk.LabelFrame(frameTabNEstr, text=" Tabellone:", borderwidth=0, highlightbackground=self.gioco_titolo_fg_color, highlightthickness=1, bg=self.gioco_tabellone_unset_bg_color, fg=self.gioco_titolo_fg_color)
        frameTabelloneBg=tk.Frame(frameTabellone, bg=self.gioco_titolo_fg_color, bd=1)
        ...
        for i in range(90):
            ...
            if i == 10:
                ...
                row = 9
                column=0
            lbl=tk.Label(frameTabelloneBg, text=i+1, font=("Helvetica", int((self.altezza/120) + (self.larghezza/190))), highlightthickness=0, highlightcolor=self.impostazioni_rbtn_circle_bg_color, width=self.larghezzaCellaTabellone, height=2, bg=self.gioco_tabellone_unset_bg_color, fg=self.gioco_tabellone_unset_fg_color) #8 / 10
            lbl.grid(row=row, column=column, sticky="nesw", pady=1, padx=1)
            tupla1 = (lbl, i+1)
            self.arrTabelloneCPU.append(lbl)
        frameTabelloneBg.grid(row=0, column=0, pady=4, padx=4)
        frameTabellone.grid(row=0, column=0)

        frameNEstratto = tk.Frame(frameTabNEstr, bg=self.gioco_sfondo_bg_color, highlightbackground=self.gioco_titolo_fg_color, highlightthickness=1)

        self.btnNEstrattoCPU = tk.Button(frameNEstratto, text="Estrai", command=lambda: self.estraiCPUSuono(), highlightthickness=0, bg=self.gioco_label_nestratto_bg_color, font=("Helvetica", int((self.altezza/76) + (self.larghezza/136)), "bold"),fg=self.gioco_label_nestratto_fg_color)
        self.btnNEstrattoCPU.grid(row=0, column=0)
        self.lblNEstrattoCPU = tk.Label(frameNEstratto, text="", fg=self.gioco_nestratto_fg_color, bg=self.gioco_nestratto_bg_color, width=2, height=1, font=('Helvetica', int((self.altezza/102) + (self.larghezza/182)))) #15
        ...
        frameSchedine = tk.LabelFrame(self.frameGiocoCPU, text=" Schedine:", borderwidth=0, bg=self.gioco_sfondo_bg_color, fg=self.gioco_schedine_unset_fg_color, highlightbackground=self.gioco_titolo_fg_color, highlightthickness=1)
        frameSchedineIn = tk.Frame(frameSchedine, borderwidth=0, bg=self.gioco_sfondo_bg_color, highlightbackground=self.gioco_titolo_fg_color, highlightthickness=0)
        self.arrSchedineCPU = []
        h = 0
        for nSchedina in range(self.nSchedineCPU.get() * 2):
            ...
            arrRow2 = []
            arrRow3 = []
            schedina = tk.LabelFrame(frameSchedineIn, text="Schedina N. "+str(int(nSchedina/2))+"           Tombolone", bg=self.gioco_tabellone_unset_bg_color, highlightbackground=self.gioco_titolo_fg_color, highlightthickness=2, borderwidth=0)
            schedinaIn = tk.LabelFrame(schedina, bg=self.gioco_titolo_fg_color, highlightbackground=self.gioco_titolo_fg_color, highlightthickness=1, borderwidth=0)
            ...
            if (nSchedina % 2) == 0:
                ...
                for riga in range(3):
                    ...
                    for colonna in range(9):
                        cella = tk.Button(schedinaIn, text="N", bg="black", disabledforeground=self.gioco_schedine_disabled_fg_color, font=("Helvetica", int((self.altezza/100) + (self.larghezza/170))), width=self.larghezzaCellaSchedine, bd=0, highlightthickness=0, relief=tk.FLAT)
                        cella.grid(row=riga, column=colonna, padx=1, pady=1)
                        if riga==0:
                    ...
                    schedinaIn.grid(row=0, column=0, padx=5, pady=5)
                self.arrSchedineCPU.append(arrSchedina)
            else:
                if self.nSchedineCPU.get() > 0 and self.nSchedineCPU.get() < 2:
                    if nSchedina != 1:
                        tk.Label(frameSchedineIn, text="  ", font=("Helvetica", int((self.altezza/307) + (self.larghezza/546))), bg=self.gioco_sfondo_bg_color).grid(row=h+1, column=0)
                elif self.nSchedineCPU.get() == 2:
                    if nSchedina != 3:
                        tk.Label(frameSchedineIn, text="  ", font=("Helvetica", int((self.altezza / 307) + (self.larghezza / 546))), bg=self.gioco_sfondo_bg_color).grid(row=h + 1, column=0)
                elif self.nSchedineCPU.get() == 3:
                    if nSchedina != 5:
                        tk.Label(frameSchedineIn, text="  ",font=("Helvetica", int((self.altezza / 307) + (self.larghezza / 546))), bg=self.gioco_sfondo_bg_color).grid(row=h + 1, column=0)

                elif self.nSchedineCPU.get() == 4:
                    print("NSHEDA: ", nSchedina)
                    if nSchedina < 3:
                        tk.Label(frameSchedineIn, text=nSchedina, font=("Helvetica", int((self.altezza/307) + (self.larghezza/546))), bg=self.gioco_sfondo_bg_color).grid(row=1, column=h)
                elif self.nSchedineCPU.get() == 5:
                    if nSchedina < 5:
                        if nSchedina != 3:
                            tk.Label(frameSchedineIn, text="  ", font=("Helvetica", int((self.altezza/307) + (self.larghezza/546))), bg=self.gioco_sfondo_bg_color).grid(row=1, column=h)
                    else:
                        if nSchedina < 7:
                            tk.Label(frameSchedineIn, text="  ", font=("Helvetica", int((self.altezza/307) + (self.larghezza/546))), bg=self.gioco_sfondo_bg_color).grid(row=h + 1, column=2)
                else:
                    if nSchedina < 5:
                        if nSchedina != 3:
                            tk.Label(frameSchedineIn, text="  ", font=("Helvetica", int((self.altezza/307) + (self.larghezza/546))), bg=self.gioco_sfondo_bg_color).grid(row=1, column=h)
                        ...
                        if nSchedina < 7:
                            tk.Label(frameSchedineIn, text="  ", font=("Helvetica", int((self.altezza/307) + (self.larghezza/546))), bg=self.gioco_sfondo_bg_color).grid(row=h + 1, column=1)
        ...
        frameSchedineIn.grid(row=0, column=0, padx=10, pady=10)
        frameSchedine.grid(row=1, column=2)

    def CreaGiocoGiocatori(self):
        #TODO*****************************************************************************************************************\
        # ******************************************  GIOCO GIOCATORI  *******************************************************\
        # *********************************************************************************************************************
        ...
        self.btnTornaAlMenuGiocoGioc=tk.Button(self.frameGiocoGioc, text="←", bg=self.gioco_sfondo_bg_color, fg=self.gioco_titolo_fg_color, highlightthickness=0, font=("Helvetica", int((self.altezza/51) + (self.larghezza/91))), bd=0, command=lambda:self.MenuSuono(), relief="solid")
        self.btnTornaAlMenuGiocoGioc.grid(row=0, column=0, columnspan=2, sticky="w")


        frameLogs=tk.LabelFrame(self.frameGiocoGioc, text=" Logs:", bg=self.gioco_logs_bg_color, fg=self.gioco_titolo_fg_color, highlightbackground=self.gioco_titolo_fg_color, highlightthickness=1, borderwidth=0)

        self.txtLogsGioc=tk.Text(frameLogs, width=45, height=6, highlightthickness=0, font=("Helvetica", int((self.altezza/85) + (self.larghezza/151))), state=tk.DISABLED, bd=0, bg=self.gioco_logs_bg_color, fg=self.gioco_logs_fg_color)
        ...
        self.btnPronto=tk.Button(self.frameGiocoGioc, text="pronto", highlightthickness=0, font=("Helvetica", int((self.altezza/85) + (self.larghezza/151)), "bold"), fg=self.gioco_sfondo_bg_color, bg=self.gioco_premi_bg_color, command=lambda: self.setPronto())
        ...
        self.btnAmboGioc=tk.Button(framePremiRimanenti, text="AMBO", highlightthickness=0, height=2, width=8, font=("Helvetica", int((self.altezza/160) + (self.larghezza/280))), bg=self.gioco_premi_btns_bg_color, fg=self.gioco_premi_btns_fg_color, disabledforeground=self.gioco_premi_btns_fg_color, state=tk.DISABLED)
        self.btnAmboGioc.grid(row=1, column=1, pady=30, sticky="nesw")

        self.btnTernaGioc=tk.Button(framePremiRimanenti, text="TERNA", highlightthickness=0, height=2, width=8, font=("Helvetica", int((self.altezza/160) + (self.larghezza/280))), bg=self.gioco_premi_btns_bg_color, fg=self.gioco_premi_btns_fg_color, disabledforeground=self.gioco_premi_btns_fg_color, state=tk.DISABLED)
        self.btnTernaGioc.grid(row=1, column=3, pady=30, sticky="nesw")

        blank2=tk.Label(framePremiRimanenti, text="", width=2, bg=self.gioco_premi_bg_color).grid(row=2, column=2)

        self.btnQuaternaGioc=tk.Button(framePremiRimanenti, text="QUATERNA", highlightthickness=0, height=2, width=8, font=("Helvetica", int((self.altezza/160) + (self.larghezza/280))), bg=self.gioco_premi_btns_bg_color, fg=self.gioco_premi_btns_fg_color, disabledforeground=self.gioco_premi_btns_fg_color, state=tk.DISABLED)
        self.btnQuaternaGioc.grid(row=2, column=1, pady=30, sticky="nesw")

        self.btnCinquinaGioc=tk.Button(framePremiRimanenti, text="CINQUINA", highlightthickness=0, height=2, width=8, font=("Helvetica", int((self.altezza/160) + (self.larghezza/290))), bg=self.gioco_premi_btns_bg_color, fg=self.gioco_premi_btns_fg_color, disabledforeground=self.gioco_premi_btns_fg_color, state=tk.DISABLED)
        ...
        self.frameClassProvGioc=tk.LabelFrame(frameDestro, text=" Classifica:", bg=self.gioco_classifica_bg_color, fg=self.gioco_classifica_titolo_fg_color, highlightbackground=self.gioco_titolo_fg_color, highlightthickness=1, borderwidth=0)
        self.frameClassProvGiocIn=tk.Frame(self.frameClassProvGioc, bg=self.gioco_classifica_bg_color)

        self.frameClassProvGiocIn.columnconfigure(0, weight=1)
        self.frameClassProvGiocIn.columnconfigure(1, weight=1)
        self.frameClassProvGiocIn.columnconfigure(2, weight=1)

        lblPos=tk.Label(self.frameClassProvGiocIn, text="Posizione", font=("Helvetica", int((self.altezza/128) + (self.larghezza/228)), "bold"), borderwidth=0.5, highlightbackground="black", relief=tk.SOLID, bg=self.gioco_classifica_bg_color, fg=self.gioco_classifica_tabella_fg_color).grid(row=0, column=0, sticky="nesw")
        lblNome=tk.Label(self.frameClassProvGiocIn, text="Nome", font=("Helvetica", int((self.altezza/128) + (self.larghezza/228)), "bold"), borderwidth=0.5, highlightbackground="black", relief=tk.SOLID, bg=self.gioco_classifica_bg_color, fg=self.gioco_classifica_tabella_fg_color).grid(row=0, column=1, sticky="nesw")
        lblPunt=tk.Label(self.frameClassProvGiocIn, text="Punteggio", font=("Helvetica", int((self.altezza/128) + (self.larghezza/228)), "bold"), borderwidth=0.5, highlightbackground="black", relief=tk.SOLID, bg=self.gioco_classifica_bg_color, fg=self.gioco_classifica_tabella_fg_color).grid(row=0, column=2, sticky="nesw")

        self.frameClassProvGiocIn.grid(row=0, column=0, padx=10, pady=10)
        ...
        # todo DA COMPATTARE
        frameTabellone = tk.LabelFrame(frameTabNEstr, text=" Tabellone:", borderwidth=0, highlightbackground=self.gioco_titolo_fg_color, highlightthickness=1,bg=self.gioco_tabellone_unset_bg_color, fg=self.gioco_titolo_fg_color)
        frameTabelloneBg = tk.Frame(frameTabellone, bg=self.gioco_titolo_fg_color)
        ...
        for i in range(90):
            ...
            if i == 10:
                ...
                row = 9
                column = 0
            lbl = tk.Label(frameTabelloneBg, text=i + 1, font=("Helvetica", int((self.altezza / 120) + (self.larghezza / 190))), highlightthickness=0, highlightcolor=self.gioco_titolo_fg_color, width=self.larghezzaCellaTabellone, height=2, bg=self.gioco_tabellone_unset_bg_color, fg=self.gioco_tabellone_unset_fg_color)  # 8 / 10
            lbl.grid(row=row, column=column, sticky="nesw", pady=1, padx=1)
            tupla1 = (lbl, i + 1)
        ...
        self.btnNEstrattoGioc = tk.Button(frameNEstratto, text="Estrai", command=lambda: self.estraiGiocSuono(), highlightthickness=0, bg=self.gioco_label_nestratto_bg_color, font=("Helvetica", int((self.altezza / 76) + (self.larghezza / 136)), "bold"), fg=self.gioco_label_nestratto_fg_color)
        self.btnNEstrattoGioc.grid(row=0, column=0)
        self.lblNEstrattoGioc = tk.Label(frameNEstratto, text="", fg=self.gioco_nestratto_fg_color, bg=self.gioco_nestratto_bg_color, width=2, height=1, font=('Helvetica', int((self.altezza / 102) + (self.larghezza / 182))))  # 15
        ...
        # Schedine segnaposto                                          SCHEDINE
        frameSchedine = tk.LabelFrame(self.frameGiocoGioc, text=" Schedine:", borderwidth=0, bg=self.gioco_sfondo_bg_color, fg=self.gioco_schedine_unset_fg_color, highlightbackground=self.gioco_titolo_fg_color, highlightthickness=2)
        frameSchedineIn = tk.Frame(frameSchedine, borderwidth=0, bg=self.gioco_sfondo_bg_color, highlightbackground=self.gioco_titolo_fg_color, highlightthickness=0)
        ...
        for nSchedina in range(self.nSchedineGioc.get() * 2):
            ...
            arrRow3 = []
            schedina = tk.LabelFrame(frameSchedineIn, text="Schedina N. " + str(int(nSchedina / 2)) + "           Tombolone", bg=self.gioco_tabellone_unset_bg_color, highlightbackground=self.gioco_titolo_fg_color, highlightthickness=2, borderwidth=0)
            schedinaIn = tk.Frame(schedina, bg=self.gioco_titolo_fg_color, highlightbackground=self.gioco_titolo_fg_color, highlightthickness=1, borderwidth=0)
            ...
            if (nSchedina % 2) == 0:
                ...
                if self.nSchedineGioc.get() > 0 and self.nSchedineGioc.get() < 2:
                    if nSchedina != 1:
                        tk.Label(frameSchedineIn, text="  ", font=("Helvetica", int((self.altezza / 307) + (self.larghezza / 546))), bg=self.gioco_sfondo_bg_color).grid(row=h + 1, column=0)
                elif self.nSchedineGioc.get() == 2:
                    if nSchedina != 3:
                        tk.Label(frameSchedineIn, text="  ", font=("Helvetica", int((self.altezza / 307) + (self.larghezza / 546))), bg=self.gioco_sfondo_bg_color).grid(row=h + 1, column=0)
                elif self.nSchedineGioc.get() == 3:
                    if nSchedina != 5:
                        tk.Label(frameSchedineIn, text="  ", font=("Helvetica", int((self.altezza / 307) + (self.larghezza / 546))), bg=self.gioco_sfondo_bg_color).grid(row=h + 1, column=0)
        ...
        frameSchedineIn.grid(row=0, column=0, pady=10, padx=10)
        frameSchedine.grid(row=1, column=2)

    def CreaTipoGioco(self):
        #TODO*****************************************************************************************************************\
        # *********************************************  TIPO GIOCO  *********************************************************\
        # *********************************************************************************************************************
    ...
    def CreaPreGiocoCPU(self):
        ...
        self.lblTitoloPreGiocoCPU = tk.Label(self.framePreGiocoCPU, text="Impostazioni Pre Gioco Con CPU", fg=self.pregioco_titolo_fg_color, bg=self.pregioco_titolo_bg_color, font=('Helvetica', int((self.altezza/30) + (self.larghezza/54)))) #50
    ...
    def CreaPreGiocoGiocatori(self):
        #TODO*****************************************************************************************************************\
        # *****************************************  PRE GIOCO GIOCATORI  ****************************************************\
        # *********************************************************************************************************************
    ...
    def CreaScegliNick(self):
        ...
        self.btnPrevNick=tk.Button(self.frameScegliNicks, text="<", highlightthickness=0, disabledforeground=self.gioco_titolo_bg_color, state=tk.DISABLED, bg=self.gioco_tabellone_unset_bg_color, fg=self.gioco_schedine_unset_fg_color, font=("Helvetica", int((self.altezza / 102) + (self.larghezza / 182)), "bold"), command=lambda: self.switchNicknameSuono("<"))
        self.btnPrevNick.grid(row=4, column=1)

        self.currNick=tk.StringVar(self, "")
        self.enNomeSceglliNick=tk.Entry(self.frameScegliNicks, textvariable=self.currNick, width=15, font=("Helvetica", int((self.altezza/102) + (self.larghezza/182))), bg=self.gioco_tabellone_unset_bg_color, fg=self.gioco_schedine_unset_fg_color, highlightthickness=0)
        self.enNomeSceglliNick.grid(row=4, column=2, columnspan=3, sticky="nesw")

        self.btnNextNick=tk.Button(self.frameScegliNicks, text=">", highlightthickness=0, disabledforeground=self.gioco_titolo_bg_color, bg=self.gioco_tabellone_unset_bg_color, fg=self.gioco_schedine_unset_fg_color, font=("Helvetica", int((self.altezza / 102) + (self.larghezza / 182)), "bold"), command=lambda: self.switchNicknameSuono(">"))
    ...
    def updateClassProvGioc(self):
        ...
        for row in range(len(giocatoriClassificaOrdinata)):
            self.plyrPos = tk.Label(self.frameClassProvGiocIn, text=((row+1),"°"), borderwidth=0.5, highlightbackground="black",relief=tk.SOLID, font=("Helvetica", int((self.altezza/128) + (self.larghezza/228))), bg=self.gioco_classifica_bg_color, fg=self.gioco_classifica_tabella_fg_color)
            self.plyrPos.grid(row=row + 1, column=column, sticky="nesw")
            self.plyrNome = tk.Label(self.frameClassProvGiocIn, text=giocatoriClassificaOrdinata[row].getNome(), borderwidth=0.5, highlightbackground="black",relief=tk.SOLID, font=("Helvetica", int((self.altezza/128) + (self.larghezza/228))), bg=self.gioco_classifica_bg_color, fg=self.gioco_classifica_tabella_fg_color)
            self.plyrNome.grid(row=row + 1, column=column + 1, sticky="nesw")
            self.plyrPunt = tk.Label(self.frameClassProvGiocIn, text=giocatoriClassificaOrdinata[row].getPunteggioCorrente(), borderwidth=0.5, highlightbackground="black",relief=tk.SOLID, font=("Helvetica", int((self.altezza/128) + (self.larghezza/228))), bg=self.gioco_classifica_bg_color, fg=self.gioco_classifica_tabella_fg_color)
    ...
    def updateClassProvCPU(self):
        ...
        column=0
        for row in range(len(giocatoriClassificaOrdinata)):
            self.plyrPos = tk.Label(self.frameClassProvCPUIn, text=((row+1),"°"), borderwidth=0.5, highlightbackground="black",relief=tk.SOLID, font=("Helvetica", int((self.altezza/128) + (self.larghezza/228))), bg=self.gioco_classifica_bg_color, fg=self.gioco_classifica_tabella_fg_color)
            self.plyrPos.grid(row=row + 1, column=column, sticky="nesw")
            self.plyrNome = tk.Label(self.frameClassProvCPUIn, text=giocatoriClassificaOrdinata[row].getNome(), borderwidth=0.5, highlightbackground="black",relief=tk.SOLID, font=("Helvetica", int((self.altezza/128) + (self.larghezza/228))), bg=self.gioco_classifica_bg_color, fg=self.gioco_classifica_tabella_fg_color)
            self.plyrNome.grid(row=row + 1, column=column + 1, sticky="nesw")
            self.plyrPunt = tk.Label(self.frameClassProvCPUIn, text=giocatoriClassificaOrdinata[row].getPunteggioCorrente(), borderwidth=0.5, highlightbackground="black",relief=tk.SOLID, font=("Helvetica", int((self.altezza/128) + (self.larghezza/228))), bg=self.gioco_classifica_bg_color, fg=self.gioco_classifica_tabella_fg_color)
            self.plyrPunt.grid(row=row + 1, column=column + 2, sticky="nesw")
    ...
    def globals(self):
        ...
        self.decina = False
        self.tombola = False
        self.MontepremiTot = self.nGiocatoriCPU.get() * self.nSchedineCPU.get()
        self.CreditiTombola = int(self.MontepremiTot / 2)
        self.CreditiCinquina = int(self.CreditiTombola / 2)
        self.CreditiQuaterna = int(self.CreditiCinquina / 2)
        self.CreditiTerna = int(self.CreditiQuaterna / 2)
        self.CreditiAmbo = int(self.CreditiTerna / 2)
    ...
    def Punteggio(self, event):
        ...
        self.frameInsertPswdInfos.grid_forget()
    ...
    def GiocaCPU(self, event):
        ...
        if flagPlay == False:
            ...
            messagebox.showerror("Attenzione!", "Non hai abbastanza crediti!")
                ...
                for k in range(len(self.arrSchedineCPU[i][l])):
                    ...
                    if self.giocatoriCpu[self.currentPlayer].getNome() == self.nicknameCPU.get():
                        ...
                        if self.giocatoriCpu[self.currentPlayer].getSchedine()[i][l][k][0] != " ":
                            self.arrSchedineCPU[i][l][k].configure(state=tk.NORMAL, bg=self.gioco_schedine_unset_bg_color)
                        else:
                            self.arrSchedineCPU[i][l][k].configure(state=tk.DISABLED, bg=self.gioco_schedine_set_bg_color)
                    else:
            ...
            #for l in range(len(self.giocatoriCpu[self.currentPlayer].getSchedine()[i])):
                ...
                #for k in range(len(self.giocatoriCpu[self.currentPlayer].getSchedine()[i][l])):
                    ...
                    if self.giocatoriGioc[self.currentPlayer].getSchedine()[i][l][k][1] == "unset":
                        ...
                        self.arrSchedineGioc[i][l][k].configure(relief=tk.SOLID)
                    ...
                    if self.giocatoriGioc[self.currentPlayer].getSchedine()[i][l][k][0] == " ":
                        self.arrSchedineGioc[i][l][k].configure(state=tk.DISABLED, bg=self.gioco_schedine_set_bg_color)
                    else:
                        self.arrSchedineGioc[i][l][k].configure(state=tk.NORMAL, bg=self.gioco_schedine_unset_bg_color)
        #print("-------------------------------------------------------------")
        ...
        if persone:
            for line in persone:
                if line != "\n":
                    ...
                    #print(gioc.getNomeLogin() + " " + gioc.getPassword() + " " , gioc.getCrediti() , " " + gioc.getNome())
        ...
        if self.Utenti != []:
            for i in range(len(self.Utenti)):
                ...
                if self.Utenti[i].getNomeLogin() == nome:
                    flagUsr=True
                if self.Utenti[i].getPassword() == password:
                    flagPswd=True
                if flagUsr and flagPswd:
                    self.nicknameCPU.set(self.Utenti[i].getNome())
                     #print("MANNAGGIA ---> " + self.nicknameCPU.get() + ", " + self.Utenti[i].getNome())
                    self.giocatore.setNome(self.Utenti[i].getNome())
                    self.giocatore.setNomeLogin(self.Utenti[i].getNomeLogin())
                    self.giocatore.setCrediti(self.Utenti[i].getCrediti())
                    self.giocatore.setPassword(self.Utenti[i].getPassword())
            if flagUsr == False:
                messagebox.showerror("Attenzione!", "Utente non trovato, \n prova a fare il signup")
            ...
            messagebox.showerror("Attenzione!", "Lista utenti vuota, \n fai prima il signup")
            ...
            if nome != "" and password1 != "" and password2 != "":
                ...
                if flag == False:
                    if password1 == password2:
                        ...
                        messagebox.showerror("Attenzione!", "Le due password non sono uguali!")
                        print("Le password non sono uguali")
                else:
                    messagebox.showerror("Attenzione", "Nome utente gia esistente, \n prova a fare il login")
                    print("Nome gia in uso")
            else:
                messagebox.showerror("Attenzione!", "Tutti i campi sono obbligatori")
    ...
    def mostraPswdProfilo(self):
        self.frameInsertPswdInfos.grid(row=2, column=1)

    def verificaPasswordInsPswd(self):
        password = self.passwordInsLogin.get()
        print("____________  "+ self.giocatore.getPassword())
        if str(self.giocatore.getPassword()) == password:
            self.Punteggio(event=None)
            self.btnInfoMostraPswd.grid_forget()
            self.enPasswordInfos.grid(row=2, column=2)
        else:
            messagebox.showerror("Attenzione!", "Password errata!")
            print("Password errata!")
    ...
    def indietroPreGiocoGioc(self, event=None):
    ...
    def clickGioc(self, giocatore, nScheda, nRiga, nCella, flag):
        ...
        if self.arrSchedineGioc[nScheda][nRiga][nCella].cget("bg") == self.gioco_schedine_unset_bg_color:
            self.arrSchedineGioc[nScheda][nRiga][nCella].configure(bg=self.gioco_schedine_set_bg_color, fg=self.gioco_schedine_set_fg_color)
            ...
            self.arrSchedineGioc[nScheda][nRiga][nCella].configure(bg=self.gioco_schedine_unset_bg_color, fg=self.gioco_schedine_unset_fg_color)
            arr = self.giocatoriGioc[self.currentPlayer].getSchedine()
            num = self.giocatoriGioc[self.currentPlayer].getSchedine()[nScheda][nRiga][nCella][0]
            commnd = "unset"
            tupla = [num, commnd]
        ...
        if flag != "cpu":
            ...
            if self.giocatoriCpu[giocatore].getSchedine()[nScheda][nRiga][nCella][0] != " ":
                if self.arrSchedineCPU[nScheda][nRiga][nCella].cget("bg") == self.gioco_schedine_unset_bg_color:
                    self.arrSchedineCPU[nScheda][nRiga][nCella].configure(bg=self.gioco_schedine_set_bg_color, fg=self.gioco_schedine_set_fg_color)
                    ...
                    self.arrSchedineCPU[nScheda][nRiga][nCella].configure(bg=self.gioco_schedine_unset_bg_color, fg=self.gioco_schedine_unset_fg_color)
                    arr = self.giocatoriCpu[self.currentPlayer].getSchedine()
                    num = self.giocatoriCpu[self.currentPlayer].getSchedine()[nScheda][nRiga][nCella][0]
                    commnd = "unset"
                    tupla = [num, commnd]
    ...
    def checkByVars(self, giocatore, nScheda, nRiga):
        ...
        if counter==2 and self.ambo is False:
            ...
            if self.giocatoriCpu[giocatore].getNome() == self.giocatore.getNome():
                self.giocatore.setCrediti(self.giocatore.getCrediti()+self.CreditiAmbo)
            ...
            self.updateClassProvCPU()
            if self.giocatoriCpu[giocatore].getNome() == self.giocatore.getNome():
                self.giocatore.setCrediti(self.giocatore.getCrediti()+self.CreditiTerna)
                self.aggiornaCreditiGiocatore(self.giocatore.getCrediti())
            ...
            self.updateClassProvCPU()
            if self.giocatoriCpu[giocatore].getNome() == self.giocatore.getNome():
                self.giocatore.setCrediti(self.giocatore.getCrediti()+self.CreditiQuaterna)
                self.aggiornaCreditiGiocatore(self.giocatore.getCrediti())
            ...
            self.updateClassProvCPU()
            if self.giocatoriCpu[giocatore].getNome() == self.giocatore.getNome():
                self.giocatore.setCrediti(self.giocatore.getCrediti()+self.CreditiCinquina)
                self.aggiornaCreditiGiocatore(self.giocatore.getCrediti())
        ...
        if counterWin1 == 15 and self.tombola is False:
            ...
            if self.verificaValiditaPremi(giocatore) == True:
                self.tombola = True
                if self.giocatoriCpu[giocatore].getNome() == self.giocatore.getNome():
                    self.giocatore.setCrediti(self.giocatore.getCrediti() + self.CreditiTombola)
                    self.aggiornaCreditiGiocatore(self.giocatore.getCrediti())
    ...
    def revealKeyBindSceglinicks(self, event=None):
        if self.btnTornaAlMenuScegliNick.cget("text") == "←":
            ...
            self.btnProntoSalvaScegliNick.configure(text="'Invio'")
        else:
            ...
            if counterProntiNicks == len(self.plyrNicks):
                self.btnProntoSalvaScegliNick.configure(text="Salva")
            else:
                self.btnProntoSalvaScegliNick.configure(text="Fatto")
    ...
    def switchNickname(self, direction):
        ...
        if self.currentNick == 0:
            ...
            self.btnNextNick.configure(bg=self.gioco_tabellone_unset_bg_color, relief=tk.RAISED)
            ...
            self.btnPrevNick.configure(bg=self.gioco_tabellone_unset_bg_color, relief=tk.RAISED)
        ...
        if counterProntiNicks != len(self.plyrNicks):
            if self.plyrNicks[self.currentNick].getFlagPronto() == False:
                self.btnProntoSalvaScegliNick.configure(bg=self.gioco_premi_titolo_fg_color, state=tk.NORMAL)
            else:
                self.btnProntoSalvaScegliNick.configure(bg=self.gioco_premi_bg_color, state=tk.DISABLED)
    ...
    def setProntoNicknames(self, event=None):
        ...
        self.btnProntoSalvaScegliNick.configure(bg="red", state=tk.DISABLED)
    ...
    def verificaProntiScegliNick(self):
        ...
        if counterProntiNicks == len(self.plyrNicks):
            self.btnProntoSalvaScegliNick.configure(state=tk.NORMAL, relief=tk.RAISED, bg=self.gioco_premi_titolo_fg_color, text="Salva", command=lambda: self.SalvaNicknames())
    ...
    def SalvaNicknames(self, event=None):
        ...
        for l in range(len(self.plyrNicks)):
            ...
            self.plyrNicks[l].setFlagPronto(False)
        self.btnProntoSalvaScegliNick.configure(state=tk.NORMAL, text="Fatto", bg=self.gioco_premi_titolo_fg_color)
        self.checkIfSettedNicks()
        self.frameOpzioniGioc.grid(row=3, column=1)
        self.frameScegliNicks.grid_forget()
