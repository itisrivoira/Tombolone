    ...
    controllaPremi() {
        ...
        for (let iPremio = 0; iPremio < contaflagRighe.length; iPremio++) {
            if (contaflagRighe[iPremio][0] == 5 && contaflagRighe[iPremio][1] == 5 && contaflagRighe[iPremio][2] == 5) {
                ...
                document.getElementById("tombola").addEventListener("click", function() { RiscattaPremioNuovo(premioTombola) });
                //this.ambo.setRiscattato(true);
                ...
                switch (contaflagRighe[iPremio][iRigaPremio]) {
                    case 2:
                        if (amboGlobale == true) {
                            ...
                            document.getElementById("quaterna").removeEventListener("click", function() { RiscattaPremioNuovo() });
                            document.getElementById("cinquina").removeEventListener("click", function() { RiscattaPremioNuovo() });
                            document.getElementById("ambo").addEventListener("click", function() { RiscattaPremioNuovo(premioAmbo) });
                            //this.ambo.setRiscattato(true);
                            this.ambo.setSchedina(iPremio);
                            this.ambo.setRiga(iRigaPremio);
                            this.flagEsistenzaPremio = true;
                    ...
                    case 3:
                        if (ternaGlobale == true) {
                            ...
                            document.getElementById("quaterna").removeEventListener("click", function() { RiscattaPremioNuovo() });
                            document.getElementById("cinquina").removeEventListener("click", function() { RiscattaPremioNuovo() });
                            document.getElementById("terna").addEventListener("click", function() { RiscattaPremioNuovo(premioTerna) });
                            //this.terna.setRiscattato(true);
                            this.terna.setSchedina(iPremio);
                            this.terna.setRiga(iRigaPremio);
                            this.flagEsistenzaPremio = true;
                    ...
                    case 4:
                        if (quaternaGlobale == true) {
                            ...
                            document.getElementById("quaterna").removeEventListener("click", function() { RiscattaPremioNuovo() });
                            document.getElementById("cinquina").removeEventListener("click", function() { RiscattaPremioNuovo() });
                            document.getElementById("quaterna").addEventListener("click", function() { RiscattaPremioNuovo(premioQuaterna) });
                            //this.quaterna.setRiscattato(true);
                            this.quaterna.setSchedina(iPremio);
                            this.quaterna.setRiga(iRigaPremio);
                            this.flagEsistenzaPremio = true;
                    ...
                    case 5:
                        if (cinquinaGlobale == true) {
                            ...
                            document.getElementById("quaterna").removeEventListener("click", function() { RiscattaPremioNuovo() });
                            document.getElementById("cinquina").removeEventListener("click", function() { RiscattaPremioNuovo() });
                            document.getElementById("cinquina").addEventListener("click", function() { RiscattaPremioNuovo(premioCinquina) });
                            //this.cinquina.setRiscattato(true);
                            this.cinquina.setSchedina(iPremio);
                            this.cinquina.setRiga(iRigaPremio);
                            this.flagEsistenzaPremio = true;
...
//flag che indicano se un premio è stato riscattato o meno
tombolaGlobale = true;
amboGlobale = true;
ternaGlobale = true;
...
let puntiIniziali;  //punti che ha l'utente prima di comprare le schedine

let montePremi; //somma del numero di schedine di tutti moltiplicato per 15
...
function avviaGioco() {
    ...
    puntiIniziali = parseInt(document.getElementById("nCreditiGiocatore").innerHTML);   //valorizzazione
    //azzero le variabili a true
    tombolaGlobale = true;
    amboGlobale = true;
    ternaGlobale = true;
    quaternaGlobale = true;
    ...
    //reset dei pulsanti con i premi per evitare che rimangano premuti
    ...
    //reset del listener nel caso l'utente torni al menu e ricrei la partita senza aggiornare la pagina
    ...
    //elimino l'eventuale tabellone della partita precedente 
    ...
    //elimino le eventuali schedine della partita precedente dell'utente
    ...
    //elimino le eventuali schedine della partita precedente delle CPU
    ...
    //ciclo dove creo tutti i giocatori con relative scedhine e la aggiungo alla lista
    ...
    //dopo aver creato i giocatori ricavo il valore dei vari premi in base alle schedine acquistate da tutti
    for (let i = 0; i < nGiocatori; i++) {
        nSchedineTutti += giocatori.getGiocatori()[i].getNumeroSchedine();
    }
    montePremi = nSchedineTutti * 15;
    premioTombola = Math.trunc(montePremi / 2);
    premioCinquina = Math.trunc(premioTombola / 2);
    premioQuaterna = Math.trunc(premioCinquina / 2);
    premioTerna = Math.trunc(premioQuaterna / 2);
    premioAmbo = Math.trunc(premioTerna / 2);
    //dopo aver creato i giocatori ricavo il valore dei vari premi in base alle schedine acquistate da tutti


    //collego i dati delle schedine creati precedentemente alla grafica creando le schedine(solo utente per inserire i listener)
    for (let i = 0; i < 1; i++) {
    ...
    //collego i dati delle schedine creati precedentemente alla grafica creando le schedine(CPU)
    for (let i = 1; i < nGiocatori; i++) {
...
function terminaTurno() {
    ...
    if (tombolaGlobale == true && giocatori.getArrayTombola().length > 0) {
        for (let i = 0; i < giocatori.getArrayTombola().length; i++) {
            giocatori.getGiocatori()[arrayTombola[i]].setPunti(premioTombola);
        }
        ...
        if (amboGlobale == true) {
            ...
            if (arrayAmboTemp.length > 0) {
                for (let i = 0; i < arrayAmboTemp.length; i++) {
                    ...
                    amboGlobale = false;
                    giocatori.getGiocatori()[arrayAmboTemp[i]].setPunti(premioAmbo);
                    document.getElementById("ambo").className = "premioSinistra";
        ...
        if (ternaGlobale == true) {
            ...
            if (arrayTernaTemp.length > 0) {
                for (let i = 0; i < arrayTernaTemp.length; i++) {
                    ...
                    ternaGlobale = false;
                    giocatori.getGiocatori()[arrayTernaTemp[i]].setPunti(premioTerna);
                    document.getElementById("terna").className = "premioSinistra";
        ...
        if (quaternaGlobale == true) {
            ...
            if (arrayQuaternaTemp.length > 0) {
                for (let i = 0; i < arrayQuaternaTemp.length; i++) {
                    ...
                    giocatori.getGiocatori()[arrayQuaternaTemp[i]].setPunti(premioQuaterna);
                    document.getElementById("quaterna").className = "premioDestra";
        ...
        if (cinquinaGlobale == true) {
            ...
            if (arrayCinquinaTemp.length > 0) {
                for (let i = 0; i < arrayCinquinaTemp.length; i++) {
                    ...
                    giocatori.getGiocatori()[arrayCinquinaTemp[i]].setPunti(premioCinquina);
                    document.getElementById("cinquina").className = "premioDestra";
...
function creaClassifica() {
    ...
    //elimino l'eventuale classifica precedente
...
function generaNumeriSchedina() {
    //funzione per creare una sola schedina
    let matriceSchedina = [];
    let flagSchedinaOk = false; //flag schedina generata correttamente
...
function posizioneCoperchioCoperto(div) {
    //funzione per spostare il coperchio in giù
...
function RiscattaPremioNuovo(num) {
    ...
    //funzione attivata dal pulsante riscatta che aggiunge al giocatore 1 il premio selezionato
    let pulsante = document.getElementById("cerchioRiscatta");
    ...
    switch (num) {
        case premioAmbo:
            ...
            pulsante.addEventListener("click", function() {
                inserisciPremioGiocatore(premioAmbo);
            })
            break;
        case premioTerna:
            ...
            document.getElementById("terna").className = "premioSinistra";
            pulsante.addEventListener("click", function() {
                inserisciPremioGiocatore(premioTerna);
            })
            break;
        case premioQuaterna:
            ...
            document.getElementById("quaterna").className = "premioDestra";
            pulsante.addEventListener("click", function() {
                inserisciPremioGiocatore(premioQuaterna);
            })
            break;
        case premioCinquina:
            ...
            document.getElementById("cinquina").className = "premioDestra";
            pulsante.addEventListener("click", function() {
                inserisciPremioGiocatore(premioCinquina);
            })
            break;
        case premioTombola:
            ...
            pulsante.addEventListener("click", function() {
                inserisciPremioGiocatore(premioTombola);
            })
...
function inserisciPremioGiocatore(premio) {
    //funzione che aggiunge ai vincitori di un determinato premio il giocatore 1
...
function finePartita(arrayGiocatori) {
    //fine partita che crea la classifica finale e il vincitore
...
function terminaPartita() {
    //crea un form invisibile con i dati da inviare al server
    let formFantasma = document.getElementById("formFantasma");
    let tagngiocatori = document.getElementById("ngiocatori");
    let nRound = document.getElementById("nRound");
    let flagVinto = document.getElementById("flagVinto");
    let nSchedine = document.getElementById("nSchedine");
    let usernameUtente = document.getElementById("usernameUtente");
    let creditiRimasti = document.getElementById("creditiRimasti");
    tagngiocatori.value = nGiocatori;
    nRound.value = (90 - numeriDisponibili.length);
    if (document.getElementById("esito").innerHTML == "VITTORIA") {
        flagVinto.value = 1;
    } else {
        flagVinto.value = 0;
    }
    nSchedine.value = giocatori.getGiocatori()[0].getNumeroSchedine();
    usernameUtente.value = giocatori.getGiocatori()[0].getNome();
    creditiRimasti.value = puntiIniziali - (giocatori.getGiocatori()[0].getNumeroSchedine() * 15) + giocatori.getGiocatori()[0].getPunti();
    formFantasma.submit();
}
